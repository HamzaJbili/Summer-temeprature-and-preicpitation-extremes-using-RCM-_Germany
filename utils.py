...
def plot_paired_trend_maps(
    ...
    """
    # ── Auto-adapt levels to the actual data range ────────────────────────────
    # Prevents one or two colours from dominating when the data span is narrower
    # than the manually-specified level range.
    ...
    if len(valid) > 10:
        ...
        n = len(levels)          # keep same number of boundaries → same colours
        if is_diverging:
